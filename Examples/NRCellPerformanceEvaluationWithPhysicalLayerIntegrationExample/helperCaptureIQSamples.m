classdef helperCaptureIQSamples < handle
    ...
    properties (Constant,Hidden)
        ...
        %BluetoothCaptureSRBW Bluetooth is captured at 82 MHz sample rate to
        %accommodate entire 80 MHz and with extra 2 MHz for better spectrum
        %visibility
        BluetoothCaptureSRBW = 82e6
    end
        ...
        function obj = helperCaptureIQSamples(node,varargin)
            ...
            if isa(node,"helperCoexNode")
                ...
                if ~isempty(node.BluetoothDevice.DeviceName)
                    ...
                    obj.Bandwidth = [obj.Bandwidth obj.BluetoothCaptureSRBW];
                    obj.pExpectedSampleRate = [obj.pExpectedSampleRate obj.BluetoothCaptureSRBW];
            ...
            elseif isa(node,"nrUE")
                ...
                obj.pDeviceNames = nodeName;
            else % bluetoothNode and bluetoothLENode
                obj.CenterFrequency = 2441e6;
                obj.Bandwidth = obj.BluetoothCaptureSRBW;
                obj.pDeviceNames = nodeName;
                obj.pExpectedSampleRate = 82e6;
            end
